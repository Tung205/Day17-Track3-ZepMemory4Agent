...
def retrieve_for_case(
    ...
    if not messages:
        try:
            ds = load_dataset()
            user = next((u for u in ds.get("users", []) if u["user_id"] == case.get("user_id")), None)
            if user:
                session = next((s for s in user.get("sessions", []) if s["thread_id"] == case.get("thread_id")), None)
                if session:
    ...
    if layer == "short_term":
        ...
        wanted.add("long_term")
    elif layer == "episodic":
        wanted.add("episodic")
    elif layer == "semantic":
        wanted.add("semantic")
    elif layer == "mixed":
        wanted.update(case.get("retrieve_layers") or ["long_term", "semantic"])
    else:
        wanted.update(["short_term", "long_term", "episodic", "semantic"])
    ...
    if "short_term" in wanted or (not wanted and short_term_text):
